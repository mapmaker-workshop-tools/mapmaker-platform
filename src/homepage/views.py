...
def contact(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            new_order.save()
            mp.track(email, "New order received" , {} )
            mp.people_set(email, {"$created": str(timezone.now)})
        ...
        mp.track("unknown user", "order page",{})
        form = Orderform()
